...
//        statService.doStat();
    ...
    private static void createExcels(String filePath) throws Exception {
        ExecutorService poolExecutor = Executors.newFixedThreadPool(16);
...
//        List<String> departmentList = Arrays.asList("中后台-营销管理中心", "中后台-数字广联达", "中后台-EMT");
        for (String department : departmentList) {
            ...
            poolExecutor.submit(new ExcelTask(department, filePath, targetFile));
        }
...
@Slf4j
@AllArgsConstructor
class ExcelTask implements Runnable {
    ...
    private static void createExcel(XSSFWorkbook targetWorkbook, String department, String filePath) {
        ...
        log.info("[{}] 填充公式", department);
        DataFillTools.fillSummarySheet(filePath + "010102.预实利润表.xlsx", targetWorkbook, department);
    }

    @SneakyThrows
    public static void after(File targetFile, XSSFWorkbook targetWorkbook) {
        try (
        ...
        log.info("Close File {}", targetFile.getAbsolutePath());
    }
}
